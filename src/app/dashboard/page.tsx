                                ...
                                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border py-2 z-30">
                                    ...
                                    {(process.env.NEXT_PUBLIC_ADMIN_EMAILS || '').split(',').map(e => e.trim()).includes(session.user?.email || '') && (
                                        <>
                                            ...
                                            <button onClick={() => router.push('/admin/analytics')} className="w-full px-4 py-2 text-sm text-left hover:bg-purple-50 flex items-center gap-2 text-purple-600"><BarChart2 size={16} /> 접속자 통계</button>
                                        </>
                                    )}
                                    <hr className="my-1" />
